...
function _drawPlayerBase(ctx, legPhase, armPhase, isJump) {
  ...
  if (isJump) {
    ...
    ctx.quadraticCurveTo(cx + 8.5, cy + 14, cx + 7, cy + 10);
    ctx.closePath();
    ctx.fill();
  } else {
    const legOff = [
    ...
    ctx.lineTo(cx + lp[0].x + 6, cy + 8 + lp[0].h);
    ...
    ctx.lineTo(cx + lp[1].x + 6, cy + 8 + lp[1].h);
    ctx.quadraticCurveTo(cx + lp[1].x + 7, cy + 8 + lp[1].h * 0.4, cx + lp[1].x + 6, cy + 8);
    ctx.closePath();
    ctx.fill();
  }
